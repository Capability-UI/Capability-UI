...
See the [examples README](https://github.com/Capability-UI/Capability-UI/tree/main/examples) for ports. The protocol workspace marketing site can host the same processes at `/play/crm/`, `/play/support/`, and `/play/agent/`.
...
Inventory and purchase orders, with a copilot on the right. Configure `OPENAI_API_KEY`, optional `OPENAI_BASE_URL`, and `OPENAI_MODEL`.
