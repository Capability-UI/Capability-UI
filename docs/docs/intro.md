...
This documentation targets the `0.1.x` TypeScript reference implementation. The core API is intentionally small and dependency-free. Authentication, identity proofing, durable policy storage, durable receipts, transport hosting, and external agent runtimes remain host integrations.
